...
function hydrateMics(shure,sennheiser){
  const configured=[...(shure.mics||[]).map(mic=>({...mic,manufacturer:"shure",channel:Number(mic.channel)||1,port:Number(mic.port)||2202})),...(sennheiser.mics||[]).map(mic=>({...mic,manufacturer:"sennheiser",channel:Number(mic.channel)||1,port:Number(mic.port)||45}))];
...
async function loadSettings(){
  ...
  const pc=settings.planning_center||{},pp=settings.propresenter||{},sh=settings.shure||{},se=settings.sennheiser||{},osm=settings.open_sound_meter||{},restream=settings.restream||{},obs=settings.obs||{},live=pc.live_from_propresenter||{};
...
function settingsPayload(){
  ...
  return {...settings,organization_name:sf.organization_name.value,timezone:sf.timezone.value,demo_mode:sf.demo_mode.checked,
    ...
    propresenter:{...(settings.propresenter||{}),enabled:sf.pp_enabled.checked,host:sf.pp_host.value,port:Number(sf.pp_port.value),remote_control_enabled:sf.pp_remote_control_enabled.checked},
    restream:{...(settings.restream||{}),enabled:sf.restream_enabled.checked,client_id:sf.restream_client_id.value.trim(),client_secret:sf.restream_client_secret.value},obs:{...(settings.obs||{}),enabled:sf.obs_enabled.checked,host:sf.obs_host.value.trim(),port:Number(sf.obs_port.value)||4455,password:sf.obs_password.value||settings.obs?.password||"",dropped_frames_threshold:Math.max(0,Number(sf.obs_dropped_frames_threshold.value)||0),preview_url:sf.obs_preview_url.value.trim()},
    shure:{...(settings.shure||{}),enabled:sf.shure_enabled.checked,receivers:[],mics:micRows.filter(mic=>(mic.manufacturer||"shure")==="shure").map(({manufacturer,...mic})=>({...mic,name:String(mic.name).trim(),host:String(mic.host).trim(),port:Number(mic.port)||2202,channel:Number(mic.channel)||1}))},sennheiser:{...(settings.sennheiser||{}),enabled:sf.sennheiser_enabled.checked,receivers:[],mics:micRows.filter(mic=>mic.manufacturer==="sennheiser").map(({manufacturer,...mic})=>({...mic,name:String(mic.name).trim(),host:String(mic.host).trim(),port:Number(mic.port)||45,channel:Number(mic.channel)||1}))},open_sound_meter:{...(settings.open_sound_meter||{}),enabled:sf.osm_enabled.checked,reports_enabled:sf.osm_reports_enabled.checked,report_weighting:sf.osm_report_weighting.value,report_response:sf.osm_report_response.value,source_id:sf.osm_source_id.value},position_mic_map:micMap};
}
...
loadDashboards();loadSettings().then(()=>{sf.pp_remote_control_enabled.checked=!!(settings.propresenter||{}).remote_control_enabled});
