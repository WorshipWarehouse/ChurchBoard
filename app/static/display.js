...
function playlistKeyboardEnabled(){return(dashboard?.widgets||[]).some(widget=>widget.type==="playlist"&&widget.settings?.keyboard_control===true)}
...
document.addEventListener("keydown",async event=>{
  if(!playlistKeyboardEnabled()||ppKeyboardInFlight||event.defaultPrevented||event.repeat||event.metaKey||event.ctrlKey||event.altKey)return;
  ...
  try{await api(`/api/integrations/propresenter/navigate/${direction}`,{method:"POST"});await refresh(true);setPlaylistKeyboardStatus("Keyboard: ←/↑ back · →/↓/Space next")}
