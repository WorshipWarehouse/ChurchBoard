...
function releaseLightingPointer(event){const button=event.target.closest("[data-lighting-button]");if(!button||button.dataset.lightingPointer!==String(event.pointerId))return;button.classList.remove("pressed");delete button.dataset.lightingPointer;sendLightingCommand(button,"release")}
document.addEventListener("pointerup",releaseLightingPointer);document.addEventListener("pointercancel",releaseLightingPointer);
document.addEventListener("keydown",event=>{const button=event.target.closest("[data-lighting-button]");if(!button||event.repeat||!["Enter"," "].includes(event.key)||button.dataset.lightingKey)return;event.preventDefault();button.dataset.lightingKey=event.key;button.classList.add("pressed");sendLightingCommand(button,"press")});
document.addEventListener("keyup",event=>{const button=event.target.closest("[data-lighting-button]");if(!button||button.dataset.lightingKey!==event.key)return;event.preventDefault();delete button.dataset.lightingKey;button.classList.remove("pressed");sendLightingCommand(button,"release")});
