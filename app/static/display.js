...
async function hydrateLightingControls(){
  ...
  try{if(!lightingButtonsCache||Date.now()-lightingButtonsLoadedAt>5000){const result=await api("/api/integrations/lighting/buttons");lightingButtonsCache=result.items||[];lightingButtonsLoadedAt=Date.now()}const pages=new Map();lightingButtonsCache.forEach(button=>{const list=pages.get(button.page)||[];list.push(button);pages.set(button.page,list)});roots.forEach(root=>{const enabled=root.dataset.lightingEnabled==="true";root.innerHTML=lightingButtonsCache.length?[...pages].map(([page,buttons])=>`<section class="lighting-page"><strong>${escapeHtml(page)}</strong><div class="control-buttons">${buttons.map(button=>`<button type="button" data-lighting-button="${escapeHtml(button.name)}" ${enabled?"":"disabled"} style="background:${safeCssColor(button.color)};color:${contrastText(button.color)}">${escapeHtml(button.name)}</button>`).join("")}</div></section>`).join(""):`<div class="empty">No pages are exposed to external applications</div>`})}catch(error){roots.forEach(root=>root.innerHTML=`<div class="empty">${escapeHtml(error.message)}</div>`)}
}
