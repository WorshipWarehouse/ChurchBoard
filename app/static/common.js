...
const partBug = (slide,label) => { const color=safeCssColor(slide.color); return `<div class="part-chip" style="--part-color:${color};--part-text:${contrastText(color)}"><span>${label}</span><strong>${escapeHtml(slide.part||"Unlabeled")}</strong></div>`; };
const slidePreview = (slide,label,{notes=false,mode="image"}={}) => {
  const image=mode==="image"&&slide.image_url?`<img class="slide-image" src="${escapeHtml(slide.image_url)}" alt="${label} slide preview">`:"";
  return `<div class="slide ${label==="Now"?"current":""} mode-${mode}"><div class="slide-label">${label}</div><div class="slide-canvas"><div class="slide-text" data-fit-slide>${escapeHtml(slide.text||"No active slide")}</div>${image}</div>${notes&&slide.notes?`<div class="slide-notes">${escapeHtml(slide.notes)}</div>`:""}</div>`;
};
...
const resizeWidgets = root => root.querySelectorAll(".widget").forEach(widget=>{const rect=widget.getBoundingClientRect();widget.classList.toggle("widget-compact",rect.height<150);widget.classList.toggle("widget-micro",rect.height<105);widget.classList.toggle("widget-narrow",rect.width<120)});
...
const resizeDashboardContent = root => {resizeWidgets(root);resizeMicCards(root);fitWidgetText(root);fitPeopleText(root)};
...
const widgetMarkup = (widget, state) => {
  ...
  if(widget.type==="slides") { const current=pp.current||{},next=pp.next||{},title=pp.planning_center_item_title||pp.title||pp.presentation?.name||pp.presentation?.id?.name||"No active item",showCurrent=settings.show_current!==false,showNext=settings.show_next!==false,mode=settings.slide_mode==="text"?"text":"image",previews=[],parts=[];if(showCurrent){previews.push(slidePreview(current,"Now",{notes:settings.show_notes!==false,mode}));parts.push(partBug(current,"Now"))}if(showNext){previews.push(slidePreview(next,"Next",{mode}));parts.push(partBug(next,"Next"))}const countSlide=showCurrent?current:showNext?next:current,countTotal=Number(countSlide.total)||0,slideCount=settings.show_slide_count&&countTotal?`<div class="pro-slide-count" data-fit-widget-text>Slide ${Number(countSlide.index)||"–"} of ${countTotal}</div>`:"";content=`<div class="pro-layout ${previews.length===1?"one-preview":previews.length===0?"no-preview":""}"><div class="pro-header"><div class="pro-title" data-fit-widget-text>${escapeHtml(title)}</div>${slideCount}</div>${previews.length?`<div class="slide-stack">${previews.join("")}</div><div class="part-strip">${parts.join("")}</div>`:`<div class="pro-hidden-message">Slide previews hidden</div>`}</div>`; }
  if(widget.type==="notes") content=`<div class="slide-notes custom-text">${escapeHtml(pp.current?.notes||"No slide notes")}</div>`;
  if(widget.type==="order") { const items=timing.service_items||service.items||[],current=timing.current_item?.id,limit=Number(settings.limit)||5;let start=Math.max(0,items.findIndex(i=>i.id===current));const serviceClock=formatClockTime(timing.service_start_at||service.starts_at,state.timezone),serviceNumber=Number(timing.service_time_count)>1?`Service ${Number(timing.service_time_index)||1} of ${Number(timing.service_time_count)}`:"Service",adjusting=["running","live","controlled"].includes(timing.state),drift=Number(timing.overall_delta||0),driftLabel=adjusting&&Math.abs(drift)>=30?`${formatDuration(drift)} ${drift>0?"late":"early"}`:"On time";content=items.length?`<div class="order-layout"><div class="order-service-time"><strong>${escapeHtml(serviceClock)}</strong><span>${escapeHtml(serviceNumber)} · ${escapeHtml(driftLabel)}</span></div><ol class="order-list">${items.slice(start,start+limit).map((item,i)=>{const leader=itemLeaderDetails(item,state),detail=settings.show_leader?`<small class="order-leader"><span>${leader?`Led by ${escapeHtml(leader.name)}`:"Leader not set"}</span>${settings.show_mic?`<em>${leader?.mic?escapeHtml(leader.mic):"No mic assigned"}</em>`:""}</small>`:"",estimate=estimatedItemTime(item,timing,state);return`<li class="${item.id===current?"active":""}"><span class="order-marker">${i?"":"▶"}</span><span class="order-main"><b>${escapeHtml(item.title)}</b>${detail}</span><span class="order-eta"><strong>${escapeHtml(estimate)}</strong><small>EST · ${Math.max(1,Math.round(Number(item.length||0)/60))}m</small></span></li>`}).join("")}</ol></div>`:`<div class="empty">No service items</div>`; }
  ...
  return `<section class="widget" data-widget="${escapeHtml(widget.id)}" data-widget-type="${escapeHtml(widget.type)}" style="grid-column:${widget.x+1}/span ${widget.w};grid-row:${widget.y+1}/span ${widget.h}"><div class="widget-heading">${escapeHtml(widget.title||widgetNames[widget.type])}</div><div class="widget-body">${content}</div></section>`;
};
const tickClocks = () => { const now=new Date(); document.querySelectorAll("[data-clock]").forEach(el=>el.textContent=now.toLocaleTimeString([],{hour:"numeric",minute:"2-digit",second:"2-digit"})); document.querySelectorAll("[data-date]").forEach(el=>el.textContent=now.toLocaleDateString([],{weekday:"long",month:"long",day:"numeric"})); requestAnimationFrame(()=>fitWidgetText(document)); };
