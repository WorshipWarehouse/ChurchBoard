...
const visibleOrderItems = (items,currentId,requestedLimit) => {
  const limit=Math.max(4,Number(requestedLimit)||5),currentIndex=items.findIndex(item=>String(item.id)===String(currentId));
  if(currentIndex<0)return items.slice(0,limit);
  const upcoming=Math.min(3,Math.max(0,items.length-currentIndex-1)),previousSlots=Math.max(0,limit-1-upcoming),start=Math.max(0,currentIndex-previousSlots);
  return items.slice(start,Math.min(items.length,start+limit));
};
...
const widgetMarkup = (widget, state) => {
  ...
  if(widget.type==="order") { const items=timing.service_items||service.items||[],current=timing.current_item?.id,visibleItems=visibleOrderItems(items,current,settings.limit);const serviceClock=formatClockTime(timing.service_start_at||service.starts_at,state.timezone),serviceNumber=Number(timing.service_time_count)>1?`Service ${Number(timing.service_time_index)||1} of ${Number(timing.service_time_count)}`:"Service",adjusting=["running","live","controlled"].includes(timing.state),drift=Number(timing.overall_delta||0),driftLabel=adjusting&&Math.abs(drift)>=30?`${formatDuration(drift)} ${drift>0?"late":"early"}`:"On time";content=items.length?`<div class="order-layout"><div class="order-service-time"><strong>${escapeHtml(serviceClock)}</strong><span>${escapeHtml(serviceNumber)} · ${escapeHtml(driftLabel)}</span></div><ol class="order-list">${visibleItems.map(item=>{const active=String(item.id)===String(current),leader=itemLeaderDetails(item,state),detail=settings.show_leader?`<small class="order-leader"><span>${leader?`Led by ${escapeHtml(leader.name)}`:"Leader not set"}</span>${settings.show_mic?`<em>${leader?.mic?escapeHtml(leader.mic):"No mic assigned"}</em>`:""}</small>`:"",estimate=estimatedItemTime(item,timing,state);return`<li class="${active?"active":""}"${active?' aria-current="step"':""}><span class="order-marker">${active?"●":""}</span><span class="order-main"><b>${escapeHtml(item.title)}${active?'<span class="order-on-air">ON AIR</span>':""}</b>${detail}</span><span class="order-eta"><strong>${escapeHtml(estimate)}</strong><small>EST · ${Math.max(1,Math.round(Number(item.length||0)/60))}m</small></span></li>`}).join("")}</ol></div>`:`<div class="empty">No service items</div>`; }
