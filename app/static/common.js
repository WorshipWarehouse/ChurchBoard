...
const propresenterPlaylistMarkup = (pp,allowTrigger=false,options={}) => {
  const slideSize=Math.max(80,Math.min(320,Number(options.slide_size)||120)),itemSize=Math.max(40,Math.min(120,Number(options.item_size)||48)),markerSize=Math.max(8,Math.min(24,Number(options.marker_size)||10)),activeBorderColor=safeCssColor(options.active_border_color||"#f5c400");
  const presentations=Array.isArray(pp.playlist_presentations)?pp.playlist_presentations:[];
  if(!presentations.length)return `<div class="empty">${escapeHtml(pp.connected===false?pp.error||"ProPresenter is disconnected":"Focus a playlist in ProPresenter to see its items")}</div>`;
  const slideMarkup=item=>{let previousPart="";return(Array.isArray(item.slides)?item.slides:[]).map(slide=>{const slideNumber=Number(slide.index),part=String(slide.part||"Unlabeled"),startsSection=part!==previousPart,isActive=String(item.presentation_uuid||"")===String(pp.presentation_uuid||"")&&slideNumber===Number(pp.current?.index);previousPart=part;const marker=startsSection?`<span class="pp-section-marker" style="--part-color:${safeCssColor(slide.color)}">${escapeHtml(part)}</span>`:"";return`<button type="button" class="pp-list-slide ${isActive?"active":""}" title="Trigger ${escapeHtml(item.title)} slide ${slideNumber}" aria-label="Trigger ${escapeHtml(item.title)} slide ${slideNumber}" ${allowTrigger?`data-pp-trigger="${slideNumber-1}" data-pp-presentation-uuid="${escapeHtml(item.presentation_uuid)}" data-pp-playlist-index="${Number(item.index)}" data-pp-is-pco="${item.is_pco?"true":"false"}"`:"disabled"}>${marker}<span class="pp-slide-number">${slideNumber}</span>${slide.image_url?`<img src="${escapeHtml(slide.image_url)}" alt="Slide ${slideNumber}">`:""}</button>`}).join("")};
  const rows=presentations.map(item=>{const placeholder=item.type==="placeholder"||item.triggerable===false;if(placeholder)return`<div class="pp-list-placeholder"><span>${Number(item.index)+1}</span><div><strong>${escapeHtml(item.title)}</strong><small>Planning Center placeholder · link content in ProPresenter</small></div></div>`;const itemSlides=slideMarkup(item),itemActive=String(item.presentation_uuid||"")===String(pp.presentation_uuid||"")||item.active;return`<section class="pp-list-presentation ${itemActive?"active":""}"><button type="button" class="pp-list-presentation-header" ${allowTrigger?`data-pp-playlist-trigger="${Number(item.index)}" data-pp-presentation-uuid="${escapeHtml(item.presentation_uuid)}" data-pp-is-pco="${item.is_pco?"true":"false"}"`:"disabled"}><span>${Number(item.index)+1}</span><strong>${escapeHtml(item.title)}</strong><small>${itemActive?"On air":`${(item.slides||[]).length} slides`}</small></button>${itemSlides?`<div class="pp-list-slides">${itemSlides}</div>`:`<div class="pp-list-slides-empty">No slides returned by ProPresenter</div>`}</section>`}).join("");
  return `<div class="pp-browser" style="--pp-slide-size:${slideSize}px;--pp-item-size:${itemSize}px;--pp-marker-size:${markerSize}px;--pp-active-border:${activeBorderColor}"><header><div><strong>${escapeHtml(pp.playlist_name||"ProPresenter Playlist")}</strong><span>${presentations.length} items</span></div><small>${escapeHtml(presentationDisplayTitle(pp))}</small></header><div class="pp-browser-scroll">${rows}</div></div>`;
};
