...
const widgetMarkup = (widget, state) => {
  ...
  if(widget.type==="order") { const items=timing.service_items||service.items||[],current=timing.current_item?.id,visibleItems=visibleOrderItems(items,current,settings.limit);const serviceClock=formatClockTime(timing.service_start_at||service.starts_at,state.timezone),serviceNumber=Number(timing.service_time_count)>1?`Service ${Number(timing.service_time_index)||1} of ${Number(timing.service_time_count)}`:"Service",adjusting=["running","live","controlled"].includes(timing.state),drift=Number(timing.overall_delta||0),driftLabel=adjusting&&Math.abs(drift)>=30?`${formatDuration(drift)} ${drift>0?"late":"early"}`:"On time";content=items.length?`<div class="order-layout"><div class="order-service-time"><strong>${escapeHtml(serviceClock)}</strong><span>${escapeHtml(serviceNumber)} · <em data-order-drift>${escapeHtml(driftLabel)}</em></span></div><ol class="order-list">${visibleItems.map(item=>{const active=String(item.id)===String(current),detail=orderLeaderMarkup(item,state,settings),estimate=estimatedItemTime(item,timing,state);return`<li class="${active?"active":""}"${active?' aria-current="step"':""}><span class="order-marker">${active?"▶":""}</span><span class="order-main"><b>${escapeHtml(item.title)}</b>${detail}</span><span class="order-duration"><strong>${formatItemLength(item.length)}</strong><small>DURATION</small></span><span class="order-eta"><strong data-order-eta data-starts-after="${Number(item.starts_after)||0}">${escapeHtml(estimate)}</strong><small>EST</small></span></li>`}).join("")}</ol></div>`:`<div class="empty">No service items</div>`; }
