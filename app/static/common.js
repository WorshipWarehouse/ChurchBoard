...
const visibleOrderItems = (items,currentId,requestedLimit) => {
  ...
  const previousSlots=Math.min(2,currentIndex),start=currentIndex-previousSlots,end=Math.min(items.length,Math.max(start+limit,currentIndex+4));
