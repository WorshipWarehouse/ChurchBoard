...
Outputs are written to `dist/installers` as an architecture-specific `.pkg` and `.dmg`.
...
GitHub Actions runs tests on pushes and pull requests. The release workflow can be started manually for test artifacts. Pushing a tag such as `v0.1.0` builds macOS Intel, macOS Apple-silicon, Windows x64, and Linux packages and attaches them to a GitHub Release.
